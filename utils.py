...
def load_f0(pitch_file, sr=44100, hop=1411):
    resolution = hop / sr
    with open(pitch_file, 'r') as f:
        pc = f.read().splitlines()
    pc = [float(p) for p in pc]
    times = np.arange(len(pc)) * resolution
    return pc, times
...
def load_lyrics(lyrics_file):
    ...
    return full_lyrics, words_lines, idx, idx_line

def write_wav(path, audio, sr):
    soundfile.write(path, audio.T, sr, "PCM_16")

def gen_phone_gt(words):
    words_p = []
    lyrics_p = []
    for word in words:
        out = g2p(word)
    ...
    return lyrics_p, words_p, idx_in_full_p
